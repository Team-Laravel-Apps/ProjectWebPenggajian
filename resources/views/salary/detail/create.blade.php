                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            ...
                            <form action="{{ route('salary.detail.store') }}" method="POST" class="form-horizontal"
    ...
    <script>
        ...
        $('input[name=lembur]').change(function() {
            ...
            if (this.checked == true) {
                ...
                let bonus = $('#transportasi').val();
                let total = parseInt(salary) - parseInt(pot_bpjs) + parseInt(bonus);
        ...
        $(document).on('keyup', '#bonus', function() {
            console.log("Bonus keyup event fired");
            HitungTotal(); // Memanggil fungsi HitungTotal untuk menghitung total gaji
            let bonus = $('#bonus').val();
            console.log("Bonus value:", bonus);
            $('#bonus_preview').html(to_rupiah(bonus)); // Memperbarui tampilan nilai bonus
        });
        ...
        $(document).on('keyup', '#gaji_lembur', function() {
            ...
            let pot_bpjs = $('#pot_bpjs').val();
            let bonus = $('#bonus').val();
            let total = parseInt(bonus) + parseInt(salary) + parseInt(total_lembur) - parseInt(pot_bpjs);
            $('#grand_total_salary').html(to_rupiah(total));
            $('#grand_total_salary_hidden').val(total);
        });
